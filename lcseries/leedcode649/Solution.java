package com.qrxedu.lcseries.leedcode649;

/**
 * Dota2 的世界里有两个阵营：Radiant(天辉)和Dire(夜魇)
 *
 * Dota2 参议院由来自两派的参议员组成。现在参议院希望对一个 Dota2 游戏里的改变作出决定。
 * 他们以一个基于轮为过程的投票进行。在每一轮中，每一位参议员都可以行使两项权利中的一项：
 *
 * 禁止一名参议员的权利：
 * 1.参议员可以让另一位参议员在这一轮和随后的几轮中丧失所有的权利。
 * 2.宣布胜利：
 *如果参议员发现有权利投票的参议员都是同一个阵营的，他可以宣布胜利并决定在游戏中的有关变化。
 *
 * 给定一个字符串代表每个参议员的阵营。字母 “R” 和 “D” 分别代表了Radiant（天辉）和Dire（夜魇）。然后，如果有 n 个参议员，给定字符串的大小将是n。
 *
 * 以轮为基础的过程从给定顺序的第一个参议员开始到最后一个参议员结束。
 *这一过程将持续到投票结束。所有失去权利的参议员将在过程中被跳过。
 *
 *假设每一位参议员都足够聪明，会为自己的政党做出最好的策略，
 * 你需要预测哪一方最终会宣布胜利并在 Dota2 游戏中决定改变。输出应该是Radiant或Dire。
 * @author qiu
 * @version 1.8.0
 */
public class Solution {
    public String predictPartyVictory(String senate) {
        for(int i = 0 ; i < senate.length();i++){
            add(senate.charAt(i));
        }
        Node cur = head;
        while(true){
            if(cur.val == 'R'){
                delete('D',cur);
            }else{
                delete('R',cur);
            }
            if(isR(cur)){
                return "Radiant";
            }
            if(isD(cur)){
                return "Dire";
            }
            cur = cur.next;
        }
    }

    /**
     * 天辉胜利的情况
     * @param head
     * @return
     */
    public boolean isR(Node head){
        Node cur = head;
        while(cur.next != head){
            if(cur.val != 'R'){
                return false;
            }
            cur = cur.next;
        }
        if(cur.val != 'R'){
            return false;
        }
        return true;
    }

    /**
     * 魇魔胜利的情况
     * @param head
     * @return
     */
    public boolean isD(Node head){
        Node cur = head;
        while(cur.next != head){
            if(cur.val != 'D'){
                return false;
            }
            cur = cur.next;
        }
        if(cur.val != 'D'){
            return false;
        }
        return true;
    }
    public Node head;
    public void add(int val){
        Node n = new Node(val);
        if(head == null){
            head = n;
            head.next = head;
        }else{
            Node cur = head;
            while(cur.next != head){
                cur = cur.next;
            }
            n.next = cur.next;
            cur.next = n;
        }
    }
    public void display(){
        Node cur= head;
        while(cur.next != head){
            System.out.print(cur.val+ " ");
            cur = cur.next;
        }
        System.out.println(cur.val);
        System.out.println();
    }

    /**
     *
     * @param val：丧失发言权力的那个人的名
     * @param tmp：某个人的最优决策是从他开始向后找到最靠近他的敌对势力并干掉他
     * @return
     */
    public Node delete(int val,Node tmp){
        Node pre = tmp;
        Node cur = tmp.next;

        while(cur.next != tmp){
            if(cur.val == val){
                Node curNext = cur.next;
                pre.next = curNext;
                cur.next = null;
                return curNext;
            }else{
                pre = pre.next;
                cur = cur.next;
            }
        }
        if(cur.val == val){
            pre.next = cur.next;
            cur.next = null;
            return pre.next;
        }
        if(cur.next.val == val){
            tmp = tmp.next;
            pre = pre.next;
            cur = cur.next;
            pre.next = cur.next;
            cur.next = null;
            return tmp;
        }
        return null;
    }
}
class Node{
    public int val;
    public Node next;

    public Node(int val) {
        this.val = val;
    }
}